...
const app = {
                    ...
                    <div class="song ${index === this.currentIndex ? 'active' : ''}" data-index="${index}">
                        ...
                        <div class="option" data-index="${index}">
                            ...
                            <div class="option-child">
                                <a class="download" href="${song.path}" download="${song.name}">
    ...
    handleEvents: function () {
        ...
        //Xử lí khi xóa bài hát trong list (phần này tưởng đơn giản nhưng để hiện màu đúng div khi xóa thì logic khá khó)
        document.addEventListener('click', function (e) {
            // Kiểm tra xem phần tử được nhấp có class 'delete' không
            if (e.target.classList.contains('delete')) {
                // Lấy giá trị data-index từ phần tử được nhấp
                const dataIndex = e.target.getAttribute('data-index');
                if ($$('.song').length > 1) {
                    deletedSong.push(Number(dataIndex));
                    console.log(deletedSong);
                    if ($('.song[data-index="' + dataIndex + '"].active')) {
                        nextBtn.onclick();
                        console.log('next when active');
                    }
                    $('.song[data-index="' + dataIndex + '"]').remove();
                    console.log('Đã xóa thẻ');
                } else {
                    alert('FBI WARNING: Please do not delete the last song');
                }
            }
        });
        ...
        prevBtn.onclick = function () {
            if (_this.isRandom) {
                ...
                _this.previousSong();
                _this.checkDeletedSongPrevious();
            }
            _this.getNewActiveSong();
        ...
        randomBtn.onclick = function () {
            ...
            randomBtn.classList.toggle('active', _this.isRandom) //ấn phát nữa thì isRandom đã tồn tại nên nó xóa cái active đi? => sai rồi, theo doc thì ở tham số thứ 2 trả về false thì nó remove, true thì nó add vào
        }
        ...
        audio.onended = function () {
            if (_this.isRepeat) {
                ...
                console.log('Hết thì tự next bài');
                nextBtn.onclick(); //hiểu là khi ended thì nó sẽ tự click next bài
        ...
        playlist.onclick = function (e) {
            ...
            //Xử lí nút 3 chấm, đoạn này làm theo logic ấn vào thì nó hiện ra, ấn vào nút ... khác thì tắt cái cũ và hiện cái mới, tức là chỉ hiện 1 cái
            if (e.target.closest('.option')) {
                const optionIndex = Number(e.target.closest('.option').getAttribute('data-index'));
                const activeOption = $('.song .option[data-index="' + optionIndex + '"] ');
                if (activeOption.classList.contains('active')) {
                    activeOption.classList.remove('active');
                } else {
                    const optionActiveElements = $$('.song .option.active');
                    optionActiveElements.forEach(optionActiveElement => {
                        optionActiveElement.setAttribute('class', 'option');
                    });
    ...
    loadConfig: function () {
        ...
        if (this.isRandom == true || this.isRepeat == true) { //set điều kiện chứ ko khi mới đăng nhập vào nó bôi màu cả 2 lên
    ...
    checkDeletedSongNext: function () {
        if (deletedSong.includes(this.currentIndex)) {
            nextBtn.onclick();
            console.log('bài đã trong arr delete nên next bài');
        }
    },
    checkDeletedSongPrevious: function () {
        if (deletedSong.includes(this.currentIndex)) {
            prevBtn.onclick();
            console.log('bài đã trong arr delete nên next bài');
        }
